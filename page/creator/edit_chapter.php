...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$user_id) { echo json_encode(['success'=>false, 'message'=>'Hết phiên đăng nhập!']); exit(); }
    ...
    } catch (Exception $e) { echo json_encode(['success'=>false, 'message'=>'Lỗi: ' . $e->getMessage()]); }
    exit();
}
...
<head>
    <meta charset="UTF-8">
    <title>Biên tập: <?php echo htmlspecialchars($story['title']); ?></title>
    ...
    <style>
        /* CSS RESET & LAYOUT */
        body { background: var(--bg-card); height: 100vh; display: flex; flex-direction: column; overflow: hidden; margin: 0; }
        ...
        /* INPUT AREA */
        .editor-meta {
            padding: 15px 30px; background: var(--bg-body); border-bottom: 1px solid var(--border);
            display: flex; gap: 15px; align-items: center;
        }
        .input-clean {
            border: 1px solid transparent; background: transparent; padding: 10px;
            font-family: inherit; color: var(--text-main); border-radius: 6px; transition: 0.2s;
        }
        .input-clean:focus { background: var(--bg-card); border-color: var(--primary); outline: none; }
        .num-inp { width: 90px; font-weight: 800; color: var(--primary); font-size: 1.1rem; text-align: center; }
        .title-inp { flex: 1; font-weight: 700; font-size: 1.1rem; }

        /* CUSTOM TOAST (Thay alert) */
        #fToast {
            position: fixed; bottom: 25px; right: 25px; background: #1e293b; color: white;
            padding: 12px 25px; border-radius: 8px; box-shadow: var(--shadow);
            transform: translateY(150px); transition: 0.4s cubic-bezier(0.18, 0.89, 0.32, 1.28); z-index: 1000;
            display: flex; align-items: center; gap: 10px; font-weight: 600;
        }
        #fToast.show { transform: translateY(0); }
        ...
        .f-modal-overlay.active { opacity: 1; visibility: visible; }
        .f-modal {
            background: var(--bg-card); width: 350px; padding: 25px; border-radius: 12px;
            text-align: center; transform: scale(0.9); transition: 0.3s;
        }
        .f-modal-overlay.active .f-modal { transform: scale(1); }
        .f-modal-btns { display: flex; gap: 10px; margin-top: 20px; }
        .btn-m { flex: 1; padding: 10px; border: none; border-radius: 6px; font-weight: 700; cursor: pointer; }
    </style>
</head>
<body>
    ...
    <div class="editor-header">
        ...
        <button onclick="saveChapter()" id="saveBtn" class="btn-write" style="background: var(--primary); color: white; border-radius: 8px;">
            <i class="fa-solid fa-cloud-arrow-up"></i> LƯU CHƯƠNG
        </button>
    </div>
    ...
    <form id="editorForm" style="display: flex; flex-direction: column; flex: 1;">
        ...
        <div class="editor-meta">
            <div style="display:flex; align-items:center; gap:8px;">
                <span style="font-size:0.75rem; font-weight:800; color:var(--text-muted);">CHƯƠNG</span>
            ...
            <input type="text" name="title" class="input-clean title-inp" value="<?php echo htmlspecialchars($chapter['title']); ?>" placeholder="Tiêu đề chương (ví dụ: Khởi đầu mới)...">
        </div>
    ...
    <div class="f-modal-overlay" id="exitModal">
        <div class="f-modal">
            <h3 style="margin:0 0 10px 0;">Rời khỏi Editor?</h3>
            <p style="font-size:0.9rem; color:var(--text-muted);">Hãy chắc chắn sếp đã lưu chương truyện trước khi thoát.</p>
    ...
    <script>
        ...
        tinymce.init({
            ...
            content_style: `
                body { font-family: 'Lora', serif; font-size: 18px; line-height: 1.8; color: #334155; padding: 40px; max-width: 700px; margin: 0 auto; }
                p { margin-bottom: 20px; }
            `,
            setup: function(editor) {
                editor.on('input change', () => {
                    document.getElementById('saveState').innerHTML = '<i class="fa-solid fa-pen-nib"></i> ĐANG SOẠN THẢO';
                });
                editor.on('keydown', (e) => {
        ...
        async function saveChapter() {
            ...
            try {
                ...
                if (data.success) {
                    toast(data.message);
                    state.innerHTML = '<i class="fa-solid fa-check"></i> ĐÃ LƯU HỆ THỐNG';
            ...
            } catch (e) { toast('Lỗi kết nối sếp ơi!'); } 
            finally { 
                btn.disabled = false; 
                btn.innerHTML = '<i class="fa-solid fa-cloud-arrow-up"></i> LƯU CHƯƠNG'; 
            }
        }

        // Logic Modal thoát
        function requestExit() { document.getElementById('exitModal').classList.add('active'); }
        function closeModal() { document.getElementById('exitModal').classList.remove('active'); }
        function goBack() { window.location.href = 'manage_story.php?id=<?php echo $story_id; ?>'; }
    </script>
</body>
</html>